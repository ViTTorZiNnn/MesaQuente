...
export function viewFor(room,uid){
 ...
 const own=c.readerId===uid,participant=c.participants.includes(uid),results=c.phase==='results',visible=results||participant&&(c.phase==='public'||own&&c.phase==='private');
 const card={};for(const k of ['id','modeId','deckIndex','readerId','participants','phase','createdAt','level','hintsRevealed','debaters'])if(c[k]!==undefined)card[k]=structuredClone(c[k]);
